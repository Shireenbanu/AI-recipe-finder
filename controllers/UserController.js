...
export async function createUser(req, res) {
  try {
    ...
    if (!email || !name)
      return res
        .status(400)
        .json({ success: false, error: "Email and name required" });

    const existingUser = await trackRDS(req, "READ_USER_EXISTENCE", () =>
    ...
    const user = await trackRDS(req, "WRITE_CREATE_USER", () =>
...
export async function getNutritionalNeeds(req, res) {
  try {
    ...
    logPerformance(
      req,
      "READ_USER_NUTRITIONAL_NEEDS_ERROR",
      Date.now() - startTime,
      "FAILURE",
      { userId },
    );
...
export async function getUserByEmail(req, res) {
  try {
    ...
    const user = await trackRDS(req, "READ_USER_BY_EMAIL", () =>
      User.getUserByEmail(email),
    );
    if (!user)
...
export async function listUserReports(req, res) {
  try {
    const { userId } = req.params;
    const result = await trackRDS(req, "READ_USER_REPORTS_LIST", () =>
      User.getUserLabReports(userId),
    );
...
export async function getUserProfile(req, res) {
  try {
    const { userId } = req.params;
    const user = await trackRDS(req, "READ_USER_PROFILE", () =>
      User.getUserById(userId),
    );
    if (!user)
      return res.status(404).json({ success: false, error: "User not found" });

    const conditions = await trackRDS(req, "READ_USER_CONDITIONS_PROFILE", () =>
      User.getUserMedicalConditions(userId),
    );
    res.json({
      success: true,
      user: { ...user, medicalConditions: conditions },
    });
  } catch (error) {
    logPerformance(
      req,
      "READ_USER_PROFILE_ERROR",
      Date.now() - startTime,
      "FAILURE",
      { userId },
    );
...
export async function addMedicalCondition(req, res) {
  try {
    ...
    const condition = await trackRDS(req, "READ_VERIFY_CONDITION", () =>
      MedicalCondition.getMedicalConditionById(conditionId),
    );
    if (!condition)
      return res
        .status(404)
        .json({ success: false, error: "Medical condition not found" });

    const userCondition = await trackRDS(req, "WRITE_ADD_USER_CONDITION", () =>
      User.addUserMedicalCondition(
        userId,
        conditionId,
        severity || "moderate",
        notes,
    ...
    if (error.code === "23505")
      return res
        .status(409)
        .json({ success: false, error: "User already has this condition" });
    logPerformance(
      req,
      "ADD_MEDICAL_CONDITION_ERROR",
      Date.now() - startTime,
      status,
      { failure_remark: error.message },
    );
    res
      .status(500)
      .json({ success: false, error: "Failed to add medical condition" });
  }
}
...
export async function updateUserProfile(req, res) {
  try {
    ...
    logPerformance(
      req,
      "WRITE_UPDATE_USER_PROFILE_ERROR",
      Date.now() - startTime,
      "FAILURE",
      { userId },
    );
...
export async function removeMedicalCondition(req, res) {
  try {
    ...
    const removed = await trackRDS(req, "WRITE_REMOVE_USER_CONDITION", () =>
      User.removeUserMedicalCondition(userId, conditionId),
    );

    if (!removed)
      return res
        .status(404)
        .json({ success: false, error: "Condition not found for this user" });
    res.json({ success: true, message: "Removed successfully" });
  } catch (error) {
    logPerformance(
      req,
      "WRITE_REMOVE_USER_CONDITION_ERROR",
      Date.now() - startTime,
      "FAILURE",
      { userId },
    );
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to remove condition" });
  }
}

...
export async function getRecommendations(req, res) {
  try {
    ...
    const { conditions, nutritionalNeeds } = await trackRDS(
      req,
      "READ_USER_NEEDS",
    ...
    const highNeedsKeys = Object.entries(nutritionalNeeds).map(
      ([key, value]) => key,
    );

    if (recipes.length < 3) {
      const newRecipes = await LLMService.generateRecipes(
        nutritionalNeeds,
        conditions,
        5,
        req,
...
export async function getRecipe(req, res) {
  try {
    ...
    const recipe = await trackRDS(req, "READ_RECIPE_BY_ID", () =>
    ...
    if (userId) {
      isFavorited = await trackRDS(req, "READ_CHECK_FAVORITE", () =>
        Recipe.isFavorited(userId, recipeId),
      );
    }
...
export async function searchRecipes(req, res) {
  try {
    ...
    if (q) {
      ...
      recipes = await trackRDS(
        req,
        "READ_SEARCH_QUERY",
        () => Recipe.searchRecipes(q, limit || 20),
        {
          search_query: q,
...
export async function addFavorite(req, res) {
  try {
    ...
    const recipe = await trackRDS(req, "READ_VERIFY_FOR_FAVORITE", () =>
      Recipe.getRecipeById(recipeId),
    );
    if (!recipe)
      return res
    ...
    const favorite = await trackRDS(req, "WRITE_ADD_FAVORITE", () =>
      Recipe.addToFavorites(userId, recipeId),
    );
...
export async function removeFavorite(req, res) {
  try {
    ...
    const removed = await trackRDS(req, "WRITE_REMOVE_FAVORITE", () =>
      Recipe.removeFromFavorites(userId, recipeId),
    );
    if (!removed)
      return res
        .status(404)
...
export async function getFavorites(req, res) {
  try {
    ...
    const favorites = await trackRDS(req, "READ_USER_FAVORITES", () =>
      Recipe.getUserFavorites(userId),
    );
    res.json({ success: true, favorites, count: favorites.length });
  } catch (error) {
    logPerformance(
      req,
      "FAVORITE_RECIPES_FETCH_FATAL",
      Date.now() - startTime,
      "FAILURE",
      {
        failure_remark: error.message,

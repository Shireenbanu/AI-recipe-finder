...
describe("Recipe API Tests", () => {
  ...
  beforeAll(async () => {
    ...
    const conditionsResponse = await request(BASE_URL).get(
      "/api/medical-conditions",
    );
    testConditionId = conditionsResponse.body.conditions[0].id;

    // Add condition to user
    await request(BASE_URL).post(`/api/users/${testUserId}/conditions`).send({
  ...
  test("GET /api/recipes/:recipeId - Get recipe by ID", async () => {
    ...
    const recResponse = await request(BASE_URL).get(
      `/api/recipes/recommendations?userId=${testUserId}`,
    );
